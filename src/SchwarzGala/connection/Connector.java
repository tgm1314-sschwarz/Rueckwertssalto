package SchwarzGala.connection;

import SchwarzGala.creator.*;
import java.sql.*;
...
public class Connector {
	...
	private createRM rm;
	private createERM erm;
	
	private String dbname= "", s="", s2="", s3="";
	...
	private ArrayList <String> pktable = new ArrayList <String>();
	...
	public void connect() throws SQLException{
		...
		while (resultTables.next()) {
			...
			while (rs.next()) {
				...
				s3 += rs.getString("FKTABLE_NAME") + " ";
			}
			fk.add(s);				//arraylist mit foreign keys füllen
			pktable.add(s2);		//arraylist mit foreign keys sortiert nach tabellen füllen
			fktable.add(s3);
			s = "";
			s2 = "";
			s3 = "";
		...
		rm = new createRM(dbname, tables, att, fk, pk, pktable);
		erm = new createERM(dbname, tables, att, fk, pk, pktable, fktable);
	}
